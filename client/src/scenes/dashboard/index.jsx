...
const Dashboard = () => {
  ...
  const totalHoursWorked =
    hoursWorkedData && Array.isArray(hoursWorkedData)
      ? hoursWorkedData.reduce(
          (sum, item) => sum + parseFloat(item.hours_worked),
          0
        )
      : 0;
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/dashboard?selectedMissingWeek=${encodeURIComponent(
            selectedMissingWeek
          )}`,
          {
            headers: {
        ...
        >
          ...
          {upcomingShifts.map((shiftInfo, i) => (
            ...
            >
              ...
              {shiftInfo.shifts.map(
                (
                  shift,
                  shiftIndex // Map over the shifts array
                ) => (
                  <Box key={shiftIndex}>
                    <Typography color={colors.grey[100]}>
                      {shift.start} bis {shift.end}{" "}
                      {/* Display the shift times */}
                    </Typography>
                  </Box>
                )
              )}
