...
const TimeReq = ({ timereq }) => {
    ...
    if (current === undefined || current === null) {
      ...
      if (closing < opening || startBreak < opening) {
        if (closing === null || closing === undefined || isNaN(closing)) {
          ...
          return (
            (current >= opening && current < startBreak) ||
            (current >= endBreak && current <= 1440) ||
            current < closing
          );
        }
      } else {
        ...
        return (
          (current >= opening && current < startBreak) ||
          (current >= endBreak && current <= closing)
        );
      }
    }
  };
    ...
    const fetchTimeReqData = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/api/requirement/workforce?week_adjustment=${weekAdjustment}&selectedDepartment=${selectedDepartment}`,
          {
            headers: {
  ...
  const applyTemplate1 = () => {
    ...
    Array.from({ length: timereqData.day_num }).forEach((_, columnIndex) => {
      Array.from({ length: timereqData.daily_slots }).forEach((_, btnIndex) => {
        const currentTimeMinutes = convertTimeToMinutes(
          timereqData.slots_dict[btnIndex]
        );
        const openingTimeMinutes = convertTimeToMinutes(
          openingHours[columnIndex]
        );
        const startBreakTimeMinutes =
          convertTimeToMinutes(startBreak[columnIndex]) - 1;
        const endBreakTimeMinutes = convertTimeToMinutes(endBreak[columnIndex]);
        const closingTimeMinutes =
          convertTimeToMinutes(closingHours[columnIndex]) - 1;

        if (
          isTimeWithinRange(
            currentTimeMinutes,
            openingTimeMinutes,
            startBreakTimeMinutes,
            endBreakTimeMinutes,
            closingTimeMinutes
          )
        ) {
  ...
  const handleFormSubmit = async (buttonName) => {
    ...
    try {
      ...
      Object.entries(slotEmployeeCounts).forEach(([key, count]) => {
        if (count !== undefined) {
          // Check if count is not undefined
  ...
  return (
    <Box m="20px">
      <Header title={t("planning.title")} subtitle={t("planning.subtitle")} />
      <Box
        sx={{
          display: "flex",
          flexDirection: "row",
      ...
      >
        <IconButton onClick={goToPreviousWeek} style={BUTTON_STYLE}>
      ...
      >
        {Array.from({ length: timereqData.day_num }).map((_, columnIndex) => (
          ...
          >
            ...
            >
              {Array.from({ length: timereqData.daily_slots }).map(
                (_, btnIndex) => {
                  ...
                  const startBreakTimeMinutes =
                    convertTimeToMinutes(startBreak[columnIndex]) - 1;
                  const endBreakTimeMinutes = convertTimeToMinutes(
                    endBreak[columnIndex]
                  );
                  const closingTimeMinutes =
                    convertTimeToMinutes(closingHours[columnIndex]) - 1;
                  ...
                  if (
                    ...
                    const isSelected = selectedButtons.includes(
                      `${columnIndex}-${btnIndex}`
                    );
                    const employeeCountForThisSlot =
                      slotEmployeeCounts[`${columnIndex}-${btnIndex}`] || "";
                    const isEntered =
                      slotEmployeeCounts[`${columnIndex}-${btnIndex}`] !==
                      undefined;
                    ...
                    return (
                      <Button
                        ...
                        onMouseDown={() =>
                          handleMouseDown(columnIndex, btnIndex)
                        }
                        onMouseOver={() => {
                          if (isDragging) {
                      ...
                      >
                        {`${
                          timereqData.slots_dict &&
                          timereqData.slots_dict[btnIndex]
                        }`}
                        {!isEntered && (
                          <span>&nbsp;&nbsp;&nbsp;{timereqValue}</span>
                        )}
                        {isEntered && (
                          <span>
                            &nbsp;&nbsp;&nbsp;{employeeCountForThisSlot}
                          </span>
                        )}
                      </Button>
                    );
