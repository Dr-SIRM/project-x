...
const Team = () => {
        ...
        >
          {departments.map((department) => (
            <MenuItem key={department} value={department}>
              <Checkbox
                checked={tempSelectedDepartments.indexOf(department) > -1}
              />
  ...
  const handleDepartmentDropdownClose = async (selectedDepartments, userId) => {
    ...
    for (let i = 0; i < 3; i++) {
      // Assuming a maximum of 3 departments
    ...
    try {
      ...
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
          user.id === userId ? { ...user, ...departmentData } : user
        )
      );
    } catch (error) {
  ...
  const getUserDepartments = (user) => {
    return [user.department, user.department2, user.department3].filter(
      Boolean
    );
  };
    ...
    <Box m="20px">
      ...
      >
        ...
        >
          ...
          >
            {t("team.deletion.confirm_delete")}
          </DialogTitle>
          <DialogContent>
            <DialogContentText
