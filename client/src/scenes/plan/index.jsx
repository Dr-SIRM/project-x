...
const GanttChart = () => {
      ...
      try {
        const response = await axios.get(`${API_BASE_URL}/api/schichtplanung`, {
          ...
          params: {
            view: view,
            ...(view === "day" && {
              specific_day: currentDay.toISOString().split("T")[0],
            }),
            ...(view === "week" && {
  ...
  const getShiftPosition = (shift) => {
    ...
    const shiftStartMinutes =
      shiftStartHourMinutes[0] * 60 + shiftStartHourMinutes[1];
    const shiftEndMinutes =
      shiftEndHourMinutes[0] * 60 + shiftEndHourMinutes[1];
      ...
      case "day":
        maxDuration =
          endMinutes < startMinutes
            ...
            : endMinutes - startMinutes;
        left =
          (((shiftStartMinutes - startMinutes + 1440) % 1440) / maxDuration) *
          100;
        width =
          (((shiftEndMinutes - shiftStartMinutes + 1440) % 1440) /
            maxDuration) *
          100;
        break;
      default:
    ...
    workersWithShiftsForDay.forEach((worker) => {
      if (worker.date === formattedDate) {
        worker.shifts.forEach((shift) => {
          if (shift.start_time) {
            // I've removed the condition to check formattedDate in start_time or end_time since it wasn't in the provided data structure. Adjust if necessary.
          }
          if (shift.end_time) {
          }
        });
          ...
          <div key={`${worker.first_name}-${worker.last_name}`}>
            ...
            {worker.shifts.map((shiftDetail) => {
              ...
              return shiftDetail.shifts.map((shift, index) => {
                const startTime =
                  shift.start_time && typeof shift.start_time === "string"
                    ? shift.start_time
                    : "N/A";
                const endTime =
                  shift.end_time && typeof shift.end_time === "string"
                    ? shift.end_time
                    : "N/A";
          ...
          <div className="modal">
            ...
            <div className="content">
              ...
              <div style={{ display: "flex", gap: "5px" }}>
                ...
                >
                  <MenuItem value={1}>
                    {t("dashboard.calendar_week")} {currentWeekNum} - Mo{" "}
                    {newDate(mondayDate, 0)}
                  </MenuItem>
                  <MenuItem value={2}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 1} - Mo{" "}
                    {newDate(mondayDate, 7)}
                  </MenuItem>
                  <MenuItem value={3}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 2} - Mo{" "}
                    {newDate(mondayDate, 14)}
                  </MenuItem>
                  <MenuItem value={4}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 3} - Mo{" "}
                ...
                >
                  <MenuItem value={1}>
                    {t("dashboard.calendar_week")} {currentWeekNum} - So{" "}
                    {newDate(mondayDate, 6)}
                  </MenuItem>
                  <MenuItem value={2}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 1} - So{" "}
                    {newDate(mondayDate, 13)}
                  </MenuItem>
                  <MenuItem value={3}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 2} - So{" "}
                    {newDate(mondayDate, 20)}
                  </MenuItem>
                  <MenuItem value={4}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 3} - So{" "}
                    {newDate(mondayDate, 27)}
                  </MenuItem>
                  <MenuItem value={5}>
                    {t("dashboard.calendar_week")} {currentWeekNum + 4} - So{" "}
                    {newDate(mondayDate, 34)}
                  </MenuItem>
                </Select>
              </div>
            </div>
          ...
          <>
            ...
            {shifts.map((workerShift, index) => (
              <div key={index} className="gantt-row">
                <Chip
                  avatar={
                    <Avatar>{`${workerShift.first_name.charAt(
                      0
                    )}${workerShift.last_name.charAt(0)}`}</Avatar>
                  }
                  label={
                    // Updated line
                    <div>
      ...
      {view === "week" && (
        <div className="week-view">
          <div className="date-navigation">
            ...
            <span className="date-display">
              Woche {getWeekNumber(currentDay)}
            </span>
          ...
          <div className="weekdays">
            {daysOfWeek(
              getStartOfWeek(currentDay),
              getEndOfWeek(currentDay)
            ).map((day, index) => {
              return (
